...
def create_crawerlist(Name, Page, crawler_list):
        url = 'https://www.google.com.hk/search?q=' + Name + \
              '+professional+profile+linkedin&dcr=0&ei=yjnXWeGUKYK98QXPmb7oBw&start=' + str(Page) + '&sa=N&biw=1440&bih=826'
        r = requests.get(url, timeout=10)
        time.sleep(5)
        if r.status_code == 200:
            soup = BeautifulSoup(r.content, "lxml")
            temp = soup.find_all('cite')
            for i in temp:
                if re.search('.*?\/in\/.*?', i.text):
                    crawler_list.append(i.text)
        else:
            print r.status_code
...
def create_csvfile(Name_Ar):
    # create crawl_list without same items
    crawler_list = []
    Max_Page = 4
    result = []
    for j in range(0, Max_Page):
        Page = (j - 1) * 10
        create_crawerlist(Name_Ar, Page, crawler_list)
    crawler_list = list(set(crawler_list))
    print crawler_list
    print len(crawler_list)
    # login and main crwaler function
    s = login(laccount='15398096133@189.cn', lpassword='jjsxqx19950513')
    for href in crawler_list:
        person_propile = {}
        crawl(href, copy.deepcopy(s), person_propile)
        if person_propile != {}:
            result.append(person_propile)
    createDictCSV(Name_Ar, result)
